...
export function TubelightNavbar({ items, className, showLogo }: NavBarProps) {
      ...
      <div className="flex items-center gap-3 bg-background/5 border border-border backdrop-blur-lg py-1 px-2 rounded-full shadow-lg">
        {showLogo && (
          <Link to="/" className="flex items-center rounded-full">
            <Icons.logo className="w-8 h-8 md:w-10 md:h-10 object-contain" />
          </Link>
        )}
        {items.map((item) => {
...
// Export as NavBar for easier importing
